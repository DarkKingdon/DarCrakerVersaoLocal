...
function inicializarCodex() {
    ...
    if (jogador.codex.luvas_simples_nivel === undefined) {
        jogador.codex.luvas_simples_nivel = 1; 
        jogador.codex.luvas_simples_progresso = 0; 
    }
...
function atualizarTelaCodex() {
    let nivelAtual = jogador.codex.luvas_simples_nivel;
    let progressoAtual = jogador.codex.luvas_simples_progresso;
    
    let qtdMochila = 0;
    if (jogador.mochila && jogador.mochila["luvas_simples"]) {
        qtdMochila = jogador.mochila["luvas_simples"].quantidade;
    }
...
function entregarLuvas(quantidade) {
    let nivelAtual = jogador.codex.luvas_simples_nivel;
    if (nivelAtual > 14) return; 

    let config = configuracaoCodex[nivelAtual];
    
    if (!jogador.mochila || !jogador.mochila["luvas_simples"] || jogador.mochila["luvas_simples"].quantidade <= 0) {
        alert("Você não possui Luvas Simples na mochila para entregar!");
        return;
    }

    let luvasDisponiveis = jogador.mochila["luvas_simples"].quantidade;
    let limiteFaltante = config.quantidadeRequerida - jogador.codex.luvas_simples_progresso;
    
    let quantidadeParaEntregar = Math.min(quantidade, luvasDisponiveis, limiteFaltante);

    jogador.mochila["luvas_simples"].quantidade -= quantidadeParaEntregar;
    jogador.codex.luvas_simples_progresso += quantidadeParaEntregar;

    if (jogador.mochila["luvas_simples"].quantidade <= 0) {
        delete jogador.mochila["luvas_simples"];
    }

    if (jogador.codex.luvas_simples_progresso >= config.quantidadeRequerida) {
        ...
        alert(`⭐ Fantástico! Você concluiu o Nível ${nivelAtual} do Códex de Luvas Simples e recebeu o bônus permanente de: ${config.desc}!`);
