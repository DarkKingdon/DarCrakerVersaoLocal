...
// Definição dos níveis do códex (14 níveis espelhados logicamente do Jellopy)
...
function inicializarCodex() {
    ...
    // Garante que a estrutura do códex exista no save do jogador de forma independente
    if (!jogador.codex) {
        jogador.codex = {};
    }
    if (jogador.codex.moedas_de_ouro_nivel === undefined) {
        jogador.codex.moedas_de_ouro_nivel = 1; 
        jogador.codex.moedas_de_ouro_progresso = 0; 
    }
...
function atualizarTelaCodex() {
    let nivelAtual = jogador.codex.moedas_de_ouro_nivel;
    let progressoAtual = jogador.codex.moedas_de_ouro_progresso;
    
    let qtdMochila = 0;
    if (jogador.mochila && jogador.mochila["moedasdeouro"]) {
        qtdMochila = jogador.mochila["moedasdeouro"].quantidade;
    }
...
function entregarMoedas(quantidade) {
    let nivelAtual = jogador.codex.moedas_de_ouro_nivel;
    if (nivelAtual > 14) return;

    let config = configuracaoCodex[nivelAtual];
    
    if (!jogador.mochila || !jogador.mochila["moedasdeouro"] || jogador.mochila["moedasdeouro"].quantidade <= 0) {
        alert("Você não possui Moedas de Ouro na mochila para entregar!");
        return;
    }

    let moedasDisponiveis = jogador.mochila["moedasdeouro"].quantidade;
    let limiteFaltante = config.quantidadeRequerida - jogador.codex.moedas_de_ouro_progresso;
    
    let quantidadeParaEntregar = Math.min(quantidade, moedasDisponiveis, limiteFaltante);

    jogador.mochila["moedasdeouro"].quantidade -= quantidadeParaEntregar;
    jogador.codex.moedas_de_ouro_progresso += quantidadeParaEntregar;

    if (jogador.mochila["moedasdeouro"].quantidade <= 0) {
        delete jogador.mochila["moedasdeouro"];
    }

    if (jogador.codex.moedas_de_ouro_progresso >= config.quantidadeRequerida) {
        ...
        // --- ATUALIZAÇÕES DIRETAS NA FICHA DO HERÓI ---
        ...
        // NÍVEL 12
        if (atributoBonus === "bonus_ataque_minimo") {
            if (jogador.ataque_minimo !== undefined) {
                jogador.ataque_minimo += valorDoBonus;
            }
        }
        // NÍVEL 13
        ...
        alert(`⭐ Fantástico! Você concluiu o Nível ${nivelAtual} do Códex de Moedas de Ouro e recebeu o bônus permanente de: ${config.desc}!`);
