...
// Definição dos níveis do códex da Maçã (14)
...
function inicializarCodex() {
    ...
    if (jogador.codex.maca_nivel === undefined) {
        jogador.codex.maca_nivel = 1; // Inicia no nível 1 do códex
        jogador.codex.maca_progresso = 0; // Quantas maçãs já foram depositadas
    }
...
function atualizarTelaCodex() {
    let nivelAtual = jogador.codex.maca_nivel;
    let progressoAtual = jogador.codex.maca_progresso;
    
    let qtdMochila = 0;
    if (jogador.mochila && jogador.mochila["maca"]) {
        qtdMochila = jogador.mochila["maca"].quantidade;
    }
...
function entregarMaca(quantidade) {
    let nivelAtual = jogador.codex.maca_nivel;
    if (nivelAtual > 14) return;

    let config = configuracaoCodex[nivelAtual];
    
    if (!jogador.mochila || !jogador.mochila["maca"] || jogador.mochila["maca"].quantidade <= 0) {
        alert("Você não possui Maçã na mochila para entregar!");
        return;
    }

    let macasDisponiveis = jogador.mochila["maca"].quantidade;
    let limiteFaltante = config.quantidadeRequerida - jogador.codex.maca_progresso;
    
    let quantidadeParaEntregar = Math.min(quantidade, macasDisponiveis, limiteFaltante);

    jogador.mochila["maca"].quantidade -= quantidadeParaEntregar;
    jogador.codex.maca_progresso += quantidadeParaEntregar;

    if (jogador.mochila["maca"].quantidade <= 0) {
        delete jogador.mochila["maca"];
    }

    if (jogador.codex.maca_progresso >= config.quantidadeRequerida) {
        ...
        alert(`⭐ Fantástico! Você concluiu o Nível ${nivelAtual} do Códex da Maçã e recebeu o bônus permanente de: ${config.desc}!`);

        jogador.codex.maca_nivel += 1;
        jogador.codex.maca_progresso = 0;
    }
