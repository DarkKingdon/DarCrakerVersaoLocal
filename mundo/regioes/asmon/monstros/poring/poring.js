...
let poring = {
    nome: "Poring",
    imagem: "../../img/monstros/poring.jpg",
    nivel: 1,
    vida_maxima: 7,
    vida_atual: 7,
    ataque_minimo: 1,
...
function selecionarFoco(tipo) {
    if (emCombate) return; // Opcional: trava troca no meio do turno automático
    focoTreino = tipo;
...
function atualizarInterfaceHP() {
    ...
    // Atualiza Poring
    let porcetagemHP_Poring = (poring.vida_atual / poring.vida_maxima) * 100;
    document.getElementById("monster-hp-bar").style.width = `${Math.max(0, porcetagemHP_Poring)}%`;
    document.getElementById("monster-hp-texto").innerText = `${poring.vida_atual}/${poring.vida_maxima}`;
}
...
function executarTurnoAutomatico() {
    ...
    // Agora somamos os bônus diretos de ataque + os novos bônus de maestria
    ...
    // --- TURNO DO JOGADOR (CONTINUA IGUAL) ---
    let defMonstro = Math.floor(Math.random() * (poring.defesa_maxima - poring.defesa_minima + 1)) + poring.defesa_minima;
    let danoFinalNoPoring = Math.max(1, danoHeroi - defMonstro);

    poring.vida_atual -= danoFinalNoPoring;
    if (poring.vida_atual < 0) poring.vida_atual = 0;

    registrarLog(`⚔️ **${jogador.nome}** atacou **Poring** e causou **${danoFinalNoPoring}** de dano!`, "#00ffff");
    atualizarInterfaceHP();

    if (poring.vida_atual <= 0) {
        vitoria();
        return;
    }

    // --- TURNO DO MONSTRO (AJUSTADO COM A PROTEÇÃO REAL E MAESTRIA DE DEFESA) ---
    setTimeout(() => {
        if (poring.vida_atual <= 0) return; 

        let danoMonstro = Math.floor(Math.random() * (poring.ataque_maximo - poring.ataque_minimo + 1)) + poring.ataque_minimo;
        ...
        // --- INTEGRAR OS BÔNUS PERMANENTES DA MAESTRIA DE DEFESA ---
        ...
        registrarLog(`💥 **Poring** contra-atacou e causou **${danoFinalNoHeroi}** de dano em você!`, "#ff0055");
        atualizarInterfaceHP();
...
function vitoria() {
    ...
    registrarLog(`🎉 **Poring foi derrotado!**`, "#22c55e");
    
    // 1. Ganho da Experiência Geral do Personagem
    jogador.experiencia_atual += poring.recompensa_exp;
    registrarLog(`✨ Você ganhou **${poring.recompensa_exp}** pontos de EXP!`, "#eab308");

    // 2. SISTEMA DE FOCO DE MAESTRIA (Ataque ou Defesa)
    if (focoTreino === "ataque") {
        jogador = ganharExpAtaque(jogador, 1); // Chama a função do nivel_ataque.js
        registrarLog(`⚔️ Você ganhou **1** ponto de EXP de Ataque!`, "#00a8ff");
    } else if (focoTreino === "defesa") {
        jogador = ganharExpDefesa(jogador, 1); // Chama a função do nivel_defesa.js
        registrarLog(`🛡️ Você ganhou **1** ponto de EXP de Defesa!`, "#22c55e");
    }

    // === VERIFICA SE SUBIU DE NÍVEL GERAL ===
    ...
    // === GARANTE QUE A MOCHILA EXISTE ANTES DOS DROPS ===
    if (!jogador.mochila) jogador.mochila = {};

    // --- SISTEMA DE DROP 1: JELLOPY ---
    let chanceDrop = Math.random() * 100;
    if (chanceDrop <= 6) { // 6% de chance
        let quantJellopy = Math.floor(Math.random() * 1) + 1; // Sorteia entre 1 e 2 unidades
    ...
    if (chanceDropMaca <= 17) { // 17% de chance
        let quantMaca = Math.floor(Math.random() * 1) + 1; // Sorteia entre 1 e 2 unidades
    ...
    if (chanceDropMoedas <= 8) { // 8% de chance
        let quantMoedas = Math.floor(Math.random() * 2) + 1; // Sorteia entre 1 e 2 unidades
    ...
    if (chanceDropZaleia <= 7) { // 7% de chance
        let quantZaleia = Math.floor(Math.random() * 1) + 1; // Sorteia entre 1 e 2 unidades
    ...
    if (chanceDropLuvas <= 4) { // 4% chance de drop
    ...
    setTimeout(() => {
        window.location.href = "poring.html";
    }, 4000);
}
...
function derrota() {
    ...
    registrarLog(`💀 Você foi derrotado pelo Poring...`, "#ff3377");
