...
// Definição dos níveis do códex do Dante (14 níveis)
...
// =========================================================================
// MECÂNICA E RENDERIZAÇÃO DO CÓDEX DO DANTE
// =========================================================================
...
function inicializarCodex() {
    ...
    // Salva nas propriedades individuais do Dante para não dar conflito com o Toby
    if (jogador.codex.dante_nivel === undefined) {
        jogador.codex.dante_nivel = 1; 
        jogador.codex.dante_progresso = 0; 
    }
...
function atualizarTelaCodex() {
    let nivelAtual = jogador.codex.dante_nivel;
    let progressoAtual = jogador.codex.dante_progresso;
    
    let qtdMochila = 0;
    // Puxa corretamente a "racao_tipo_1" da mochila do herói
...
function entregarRacao(quantidade) {
    ...
    // Altera validação para buscar "racao_tipo_1"
    if (!jogador.mochila || !jogador.mochila["racao_tipo_1"] || jogador.mochila["racao_tipo_1"].quantidade <= 0) {
        alert("Você não possui Ração Tipo 1 na mochila para alimentar o Dante!");
        return;
    }

    let racoesDisponiveis = jogador.mochila["racao_tipo_1"].quantidade;
    let limiteFaltante = config.quantidadeRequerida - jogador.codex.dante_progresso;
    ...
    jogador.codex.dante_progresso += quantidadeParaEntregar;

    if (jogador.mochila["racao_tipo_1"].quantidade <= 0) {
        delete jogador.mochila["racao_tipo_1"];
    }

    if (jogador.codex.dante_progresso >= config.quantidadeRequerida) {
        ...
        if (atributoBonus === "bonus_ataque_minimo") {
            if (jogador.ataque_minimo !== undefined) jogador.ataque_minimo += valorDoBonus;
        }
        ...
        alert(`⭐ Fantástico! O Dante subiu para o Nível ${nivelAtual} e você recebeu o bônus permanente de: ${config.desc}!`);
