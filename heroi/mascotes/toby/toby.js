...
// Definição dos níveis do códex do Toby (14 níveis com bônus idênticos aos do Jellopy)
...
// =========================================================================
// MECÂNICA E RENDERIZAÇÃO DO CÓDEX DO TOBY
// =========================================================================
...
function inicializarCodex() {
    ...
    if (jogador.codex.toby_nivel === undefined) {
        jogador.codex.toby_nivel = 1; // Inicia no nível 1 do Toby
        jogador.codex.toby_progresso = 0; // Quantas rações já foram depositadas
    }
...
function atualizarTelaCodex() {
    let nivelAtual = jogador.codex.toby_nivel;
    let progressoAtual = jogador.codex.toby_progresso;
...
function entregarRacao(quantidade) {
    ...
    if (!jogador.mochila || !jogador.mochila["racao_tipo_1"] || jogador.mochila["racao_tipo_1"].quantidade <= 0) {
        alert("Você não possui Ração Tipo 1 na mochila para alimentar o Toby!");
        return;
    }

    let racoesDisponiveis = jogador.mochila["racao_tipo_1"].quantidade;
    let limiteFaltante = config.quantidadeRequerida - jogador.codex.toby_progresso;
    ...
    jogador.codex.toby_progresso += quantidadeParaEntregar;

    if (jogador.mochila["racao_tipo_1"].quantidade <= 0) {
        delete jogador.mochila["racao_tipo_1"];
    }

    if (jogador.codex.toby_progresso >= config.quantidadeRequerida) {
        ...
        alert(`⭐ Fantástico! O Toby subiu para o Nível ${nivelAtual} e você recebeu o bônus permanente de: ${config.desc}!`);

        jogador.codex.toby_nivel += 1;
        jogador.codex.toby_progresso = 0;
    }
