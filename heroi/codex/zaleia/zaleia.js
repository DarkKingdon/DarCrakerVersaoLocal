...
// Definição dos níveis do códex (14 níveis espelhados do Jellopy)
...
function inicializarCodex() {
    ...
    if (jogador.codex.zaleia_nivel === undefined) {
        jogador.codex.zaleia_nivel = 1; // Inicia no nível 1 do códex
        jogador.codex.zaleia_progresso = 0; // Quantas zaleias já foram depositadas
    }
...
function atualizarTelaCodex() {
    let nivelAtual = jogador.codex.zaleia_nivel;
    let progressoAtual = jogador.codex.zaleia_progresso;
    
    let qtdMochila = 0;
    if (jogador.mochila && jogador.mochila["zaleia"]) {
        qtdMochila = jogador.mochila["zaleia"].quantidade;
    }
...
function entregarZaleia(quantidade) {
    let nivelAtual = jogador.codex.zaleia_nivel;
    if (nivelAtual > 14) return;

    let config = configuracaoCodex[nivelAtual];
    
    if (!jogador.mochila || !jogador.mochila["zaleia"] || jogador.mochila["zaleia"].quantidade <= 0) {
        alert("Você não possui Zaleia na mochila para entregar!");
        return;
    }

    let zaleiasDisponiveis = jogador.mochila["zaleia"].quantidade;
    let limiteFaltante = config.quantidadeRequerida - jogador.codex.zaleia_progresso;
    
    let quantidadeParaEntregar = Math.min(quantidade, zaleiasDisponiveis, limiteFaltante);

    jogador.mochila["zaleia"].quantidade -= quantidadeParaEntregar;
    jogador.codex.zaleia_progresso += quantidadeParaEntregar;

    if (jogador.mochila["zaleia"].quantidade <= 0) {
        delete jogador.mochila["zaleia"];
    }

    if (jogador.codex.zaleia_progresso >= config.quantidadeRequerida) {
        ...
        alert(`⭐ Fantástico! Você concluiu o Nível ${nivelAtual} do Códex da Zaleia e recebeu o bônus permanente de: ${config.desc}!`);

        jogador.codex.zaleia_nivel += 1;
        jogador.codex.zaleia_progresso = 0;
    }
